...
series_df <- matrix(c(prog1,
                      ...
                      prog18),
                    nrow = length(prog1),
                    dimnames = list(paste0("t_", 1:length(prog1)),
                                    paste0("s_", 1:18)))
write.csv(series_df, file = sprintf("%ssample_series.csv", outDir))
...
plot(d)
stl_result <- stl(d, s.window = "periodic")
plot(stl_result)
